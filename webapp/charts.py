...
def resting_hr_trend_svg(data: dict, width: int = 900, height: int = 160) -> str:
    """Plain resting-HR line over time, date-offset positioned so real
    logging gaps show as real gaps - no target line, no color-by-status;
    the judgment call already lives elsewhere, this is just the shape."""
    points = data["points"]
    n = len(points)
    if n == 0:
        return f"<svg width='{width}' height='{height}'></svg>"

    values = [p["resting_hr"] for p in points]
    lo, hi = min(values), max(values)
    pad_val = (hi - lo) * 0.2 or 1
    lo, hi = lo - pad_val, hi + pad_val
    ...
    line_points = [(x_of(p["date"]), y_of(p["resting_hr"])) for p in points]
    if len(line_points) >= 2:
        path = "M " + " L ".join(f"{x:.1f} {y:.1f}" for x, y in line_points)
        parts.append(f"<path d='{path}' fill='none' stroke='{NEUTRAL}' stroke-width='1.5' opacity='0.8'/>")
...
def nutrient_trend_svg(points: list, unit: str, width: int = 280, height: int = 90) -> str:
    """Small-multiple sparkline-style trend for one nutrient - deliberately
    compact (this runs once per tracked nutrient on the Trends page), no
    axis labels beyond first/last date, just the shape of the last N days."""
    n = len(points)
    if n == 0:
        return f"<svg width='{width}' height='{height}'></svg>"
    ...
    last_p = points[-1]
    last_x, last_y = line_points[-1]
    tip = _tip([_human_date(last_p["date"]), f"{last_p['value']}{unit}"])
    parts.append(f"<circle data-tip='{tip}' cx='{last_x:.1f}' cy='{last_y:.1f}' r='2.5' fill='{TEXT}'/>")

    parts.append(f"<text x='{pad_left}' y='{height - 5}' {LABEL_FONT} font-size='9' fill='{DIM}'>{_esc(_human_date(points[0]['date']))}</text>")
    parts.append(f"<text x='{width - pad_right}' y='{height - 5}' {LABEL_FONT} font-size='9' fill='{DIM}' text-anchor='end'>{_esc(_human_date(points[-1]['date']))}</text>")
