...
def resting_hr_trend_svg(data: dict, width: int = 900, height: int = 200) -> str:
    """Raw daily readings as faint dots (real logging gaps show as real
    gaps - date-offset positioned, not index) with a bold 7-day-EWMA line
    through them, same "faint raw + bold smoothed" language as
    weight_trend_svg - the rolling average is what actually answers
    "is this trending anywhere," not the noisy daily scatter alone."""
    points = data["points"]
    n = len(points)
    if n == 0:
        return f"<svg width='{width}' height='{height}'></svg>"

    raw_values = [p["resting_hr"] for p in points]
    ewma_values = [p["ewma"] for p in points]
    ...
    avg_y = y_of(data["avg"])
    parts.append(f"<line x1='{pad_left}' y1='{avg_y:.1f}' x2='{width - pad_right}' y2='{avg_y:.1f}' stroke='{MUTED}' stroke-width='1' stroke-dasharray='3 3' opacity='0.7'/>")
    parts.append(f"<text x='{pad_left + 4}' y='{avg_y - 4:.1f}' {LABEL_FONT} font-size='9.5' fill='{MUTED}'>window avg {data['avg']}</text>")

    for p in points:
        x, y = x_of(p["date"]), y_of(p["resting_hr"])
        tip = _tip([_human_date(p["date"]), f"Resting HR {p['resting_hr']}"])
        parts.append(f"<circle data-tip='{tip}' cx='{x:.1f}' cy='{y:.1f}' r='2.5' fill='{MUTED}' opacity='0.6'/>")

    ewma_points = [(x_of(p["date"]), y_of(p["ewma"])) for p in points]
    path = "M " + " L ".join(f"{x:.1f} {y:.1f}" for x, y in ewma_points)
    parts.append(f"<path d='{path}' fill='none' stroke='{NEUTRAL}' stroke-width='2'/>")

    last_x, last_y = ewma_points[-1]
    parts.append(f"<text x='{last_x:.1f}' y='{last_y - 10:.1f}' {FONT} font-size='12' fill='{TEXT}' text-anchor='end'>{ewma_values[-1]:.1f}</text>")
...
def training_load_trend_svg(data: dict, width: int = 900, height: int = 160) -> str:
    """Acute:chronic training-load ratio, one point per day (dense/regular,
    so index-spaced not date-offset). A dashed reference line at 1.5 marks
    the same "flag_high" threshold acute_chronic_load_ratio() already uses
    elsewhere - above it means load is ramping fast relative to the last
    month; nothing below 0.8 is flagged since undertraining isn't the risk
    ...
    values = [p["ratio"] for p in points]
    lo, hi = min(0.5, min(values)), max(1.6, max(values))
    pad_val = (hi - lo) * 0.1 or 0.1
    lo, hi = lo - pad_val, hi + pad_val
    ...
    for g in (0.8, 1.0, 1.5):
        gy = y_of(g)
        dash = "stroke-dasharray='4 3'" if g == 1.5 else ""
        stroke = WARN if g == 1.5 else LINE_SOFT
        parts.append(f"<line x1='{pad_left}' y1='{gy:.1f}' x2='{width - pad_right}' y2='{gy:.1f}' stroke='{stroke}' stroke-width='1' {dash}/>")
        parts.append(f"<text x='{width - pad_right + 8}' y='{gy + 4:.1f}' {FONT} font-size='10.5' fill='{DIM}'>{g}</text>")

    if "avg" in data:
        avg_y = y_of(data["avg"])
        parts.append(f"<line x1='{pad_left}' y1='{avg_y:.1f}' x2='{width - pad_right}' y2='{avg_y:.1f}' stroke='{MUTED}' stroke-width='1' stroke-dasharray='2 2' opacity='0.7'/>")
        parts.append(f"<text x='{pad_left + 4}' y='{avg_y - 4:.1f}' {LABEL_FONT} font-size='9.5' fill='{MUTED}'>window avg {data['avg']}</text>")

    line_points = [(x_of(i), y_of(p["ratio"])) for i, p in enumerate(points)]
    path = "M " + " L ".join(f"{x:.1f} {y:.1f}" for x, y in line_points)
    parts.append(f"<path d='{path}' fill='none' stroke='{NEUTRAL}' stroke-width='1.5' opacity='0.85'/>")

    tip = _tip([_human_date(points[-1]["date"]), f"Ratio {points[-1]['ratio']}"])
    last_x, last_y = line_points[-1]
    parts.append(f"<circle data-tip='{tip}' cx='{last_x:.1f}' cy='{last_y:.1f}' r='3.5' fill='{TEXT}'/>")
...
_NUTRIENT_STATUS_COLOR = {"good": AHEAD, "near": WARN, "under": BEHIND, "over": BEHIND}


def nutrient_trend_svg(points: list, unit: str, goal: float = None, status: str = None,
                        width: int = 280, height: int = 130) -> str:
    """Small-multiple trend for one nutrient. A dashed goal line (when a
    goal exists) gives the sparkline something to read against instead of
    just its own shape, the line is colored by the window's good/near/
    under/over status (same colors as the Nutrition page's nutrient bars),
    and the y-axis carries real min/max labels rather than being unscaled
    and unlabeled."""
    n = len(points)
    if n == 0:
        return f"<svg width='{width}' height='{height}'></svg>"
    ...
    pad_val = (hi - lo) * 0.15 or (hi * 0.1 or 1)
    lo, hi = max(lo - pad_val, 0), hi + pad_val

    pad_left, pad_right, pad_top, pad_bottom = 4, 34, 10, 18
    plot_w = width - pad_left - pad_right
    plot_h = height - pad_top - pad_bottom
    ...
    parts = [f"<svg viewBox='0 0 {width} {height}' preserveAspectRatio='none' style='width:100%;height:100%;display:block'>"]
    parts.append(f"<rect width='{width}' height='{height}' fill='{GROUND}' rx='4'/>")

    parts.append(f"<text x='{width - pad_right + 4}' y='{pad_top + 4}' {FONT} font-size='9' fill='{DIM}'>{hi - pad_val:.0f}</text>")
    parts.append(f"<text x='{width - pad_right + 4}' y='{pad_top + plot_h:.1f}' {FONT} font-size='9' fill='{DIM}'>{lo + pad_val:.0f}</text>")

    if goal is not None:
        goal_y = y_of(goal)
        parts.append(f"<line x1='{pad_left}' y1='{goal_y:.1f}' x2='{pad_left + plot_w:.1f}' y2='{goal_y:.1f}' stroke='{MUTED}' stroke-width='1' stroke-dasharray='3 3' opacity='0.7'/>")
        parts.append(f"<text x='{width - pad_right + 4}' y='{goal_y + 3:.1f}' {LABEL_FONT} font-size='8.5' fill='{MUTED}'>goal</text>")

    line_color = _NUTRIENT_STATUS_COLOR.get(status, NEUTRAL)
    line_points = [(x_of(i), y_of(p["value"])) for i, p in enumerate(points)]
    if len(line_points) >= 2:
        path = "M " + " L ".join(f"{x:.1f} {y:.1f}" for x, y in line_points)
        parts.append(f"<path d='{path}' fill='none' stroke='{line_color}' stroke-width='1.5' opacity='0.9'/>")

    for p, (x, y) in zip(points, line_points):
        tip = _tip([_human_date(p["date"]), f"{p['value']}{unit}"])
        parts.append(f"<circle data-tip='{tip}' cx='{x:.1f}' cy='{y:.1f}' r='2' fill='{MUTED}' opacity='0.5'/>")

    last_x, last_y = line_points[-1]
    parts.append(f"<circle cx='{last_x:.1f}' cy='{last_y:.1f}' r='2.5' fill='{line_color}'/>")

    parts.append(f"<text x='{pad_left}' y='{height - 5}' {LABEL_FONT} font-size='9' fill='{DIM}'>{_esc(_human_date(points[0]['date']))}</text>")
    parts.append(f"<text x='{pad_left + plot_w:.1f}' y='{height - 5}' {LABEL_FONT} font-size='9' fill='{DIM}' text-anchor='end'>{_esc(_human_date(points[-1]['date']))}</text>")
